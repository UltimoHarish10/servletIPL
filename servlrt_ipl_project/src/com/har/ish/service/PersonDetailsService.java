...
public class PersonDetailsService {
	...
	public List<AllPersonalDetailsDto> getAllPersonalDetails(AllPersonalDetailsDto personDto){
	...
	public List<AllPersonalDetailsDto> getPersonDetailsByFilters(FilterDto filterDto,List<AllPersonalDetailsDto> personDetails){
	...
	public XSSFWorkbook exportWorkSheet(XSSFWorkbook workBook, List<AllPersonalDetailsDto> personalDetails) {
		...
		try {
			if (personalDetails != null && !personalDetails.isEmpty()) {
				...
				cell = row.createCell(colNum);
				cell.setCellStyle(cellStyle);
				cell.setCellValue(CommonMethods.AGE);
				colNum++;
				sheet1.setColumnWidth(colNum, CommonMethods.SEVENTHOUSANDFIVEHUNDRED);
				cell = row.createCell(colNum);
				cell.setCellStyle(cellStyle);
					...
					if (totalSize != 0) {
						...
						for (int i = 0; i < totalSize; i++) {
							...
							if (person.getAge() != null) {
								cells.setCellValue(person.getAge());
							} else {
								cells.setCellValue(CommonMethods.HYPHEN);
							}
							currCol++;
